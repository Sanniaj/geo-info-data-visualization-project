"""
Download RAW USGS _DEM for California across the entire year 2020.
- uncleaned and saves each 10-day chunk as returned by USGS.
- Output files: data/usgs/firms_CA_<SOURCE>_<YYYY-MM-DD>.csv
"""
...
from config import USGS_API_KEY, USGS_API_URL, CA_BBOX_STR, USGS_DATA_DIR
...
def download_chunk(start_dt: date, days: int):
    ...
    url = (
        f"{USGS_API_URL}area/csv/"
        f"{USGS_API_KEY}/{SOURCE}/{CA_BBOX_STR}/{days}/{start_str}"
    )
    print(f"[{start_str} +{days}d] GET {url}")
    df = pd.read_csv(url)
    out_name = f"firms_CA_{SOURCE}_{start_str}.csv"
    out_path = os.path.join(USGS_DATA_DIR, out_name)
    df.to_csv(out_path, index=False)
    print(f"  Saved {len(df)} rows → {out_path}")

def main():
    if not USGS_API_KEY:
        raise SystemExit("Missing USGS_API_KEY. Set it in .env or config.py.")
